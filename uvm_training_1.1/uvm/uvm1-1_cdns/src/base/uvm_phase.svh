...
extern function void ml_execute_func_phase(uvm_component top);
extern static function void m_prepare_for_run_phases();
extern static task m_iterate_through_phases();
extern static function void m_ml_run_phases(uvm_component top);
...
function void uvm_phase::ml_execute_func_phase(uvm_component top);

  uvm_root rt;
  bit is_top_root;
  uvm_phase tmp_phase;

  if ($cast(rt, top)) is_top_root = 1;
...
//      phase_done = uvm_test_done_objection::get();
    // getting out the (pre-run) phase from the hopper for ML environment which will run the phase from outside
    if (m_phase_hopper.try_get(tmp_phase) == 0)
      // TBD: issue an error
      uvm_report_fatal("CANTRTRV",
                       { "Can't retrieve phase ", this.get_name(), " from hopper." });

    if  (tmp_phase != this)
      uvm_report_fatal("INCRCTPHS",
                       { "Incorret phase. Expecting to get phase ", this.get_name(), " from hopper, but got ", tmp_phase.get_name(), "." });
...
*/
  ...
  // If more successors, schedule them to run now
  m_phase_top_procs.delete(this);
  if (m_successors.size() == 0 && is_top_root) begin
    rt.m_phase_all_done=1;
  end 
  else begin
    // execute all the successors
    foreach (m_successors[succ]) begin
      if(succ.m_state < UVM_PHASE_SCHEDULED) begin
        ...
        void'(m_phase_hopper.try_put(succ));
        if (m_phase_trace)
          `UVM_PH_TRACE("PH/TRC/SCHEDULED",{"Scheduling phase ", get_full_name()},succ,UVM_LOW)
...
function void uvm_phase::m_ml_run_phases(uvm_component top);
  ...
  m_prepare_for_run_phases();
  void'(m_phase_hopper.try_get(common));
  common.ml_execute_func_phase(top);
endfunction
