...
`ifdef UVM_PACK_MAX_SIZE
  `define UVM_PACKER_MAX_BYTES `UVM_PACK_MAX_SIZE
`endif
`ifndef UVM_PACKER_MAX_BYTES
  `define UVM_PACKER_MAX_BYTES UVM_STREAMBITS
`endif
`ifndef UVM_PACK_MAX_SIZE
  `define UVM_PACK_MAX_SIZE `UVM_PACKER_MAX_BYTES
`endif
//--------------------
// ML Additions (END)
//--------------------
...
function void uvm_packer::pack_field_int(logic [63:0] value, int size);
  ...
  // (in order to save runtime performance the code is duplicated there)
  for (int i=0; i<size; i++)
    if(big_endian == 1)
      m_bits[count+i] = value[size-1-i];
    else
