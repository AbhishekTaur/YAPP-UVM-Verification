...
`define UVM_BLOCKING_PUT_IMP(imp, TYPE, arg) \
  task put( input TYPE arg ); \
    if (external_connector != null) begin \
      uvm_object tmp; \
      $cast(tmp,arg); \
      external_connector.put(tmp); \
    end else imp.put( arg ); \
  endtask
...
`define UVM_NONBLOCKING_PUT_IMP(imp, TYPE, arg) \
  function bit try_put (TYPE arg); \
    ...
    if (external_connector != null) begin \
      uvm_object tmp; \
      $cast(tmp, arg); \
      ret_val = external_connector.try_put(tmp); \
    end else \
      if (imp != null) ret_val = imp.try_put( arg ); \
      else ret_val = 0; \
    return ret_val; \
  endfunction \
  function bit can_put(); \
    ...
    if (external_connector != null) \
      ret_val = external_connector.can_put(); \
    else \
      if (imp != null) ret_val = imp.can_put(); \
      else ret_val = 0; \
    return ret_val; \
  endfunction
...
`define UVM_BLOCKING_GET_IMP(imp, TYPE, arg) \
  task get (output TYPE arg); \
    if (external_connector != null) begin \
      uvm_object tmp; \
      external_connector.get(tmp); \
      $cast(arg, tmp); \
    end else imp.get(arg); \
  endtask

`define UVM_NONBLOCKING_GET_IMP(imp, TYPE, arg) \
  function bit try_get (output TYPE arg); \
    bit ret_val; \
    if (external_connector) begin \
      uvm_object tmp; \
      ret_val = external_connector.try_get( tmp ); \
      $cast(arg, tmp); \
    end else \
      if (imp != null) ret_val = imp.try_get( arg ); \
      else ret_val = 0; \
    return ret_val; \
  endfunction \
  function bit can_get(); \
    bit ret_val; \
    if (external_connector != null) ret_val = external_connector.can_get(); \
    else \
      if (imp != null) ret_val = imp.can_get(); \
      else ret_val = 0; \
    return ret_val; \
  endfunction
...
`define UVM_BLOCKING_PEEK_IMP(imp, TYPE, arg) \
  task peek (output TYPE arg); \
    if (external_connector != null) begin \
      uvm_object tmp; \
      external_connector.peek( tmp ); \
      $cast(arg, tmp); \
    end else imp.peek(arg); \
  endtask
...
`define UVM_NONBLOCKING_PEEK_IMP(imp, TYPE, arg) \
  function bit try_peek (output TYPE arg); \
    ...
    if (external_connector != null) begin \
      uvm_object tmp; \
      ret_val = external_connector.try_peek( tmp ); \
      $cast(arg, tmp); \
    end else \
      if (imp != null) ret_val = imp.try_peek( arg ); \
      else ret_val = 0; \
    return ret_val; \
  endfunction \
  function bit can_peek(); \
    bit ret_val; \
    if (external_connector != null) ret_val = external_connector.can_peek(); \
    else \
      if (imp != null) ret_val = imp.can_peek(); \
      else ret_val = 0; \
    return ret_val; \
  endfunction
...
`define UVM_BLOCKING_TRANSPORT_IMP(imp, REQ, RSP, req_arg, rsp_arg) \
  task transport (REQ req_arg, output RSP rsp_arg); \
    ...
    end else imp.transport(req_arg, rsp_arg); \
  endtask
...
`define UVM_NONBLOCKING_TRANSPORT_IMP(imp, REQ, RSP, req_arg, rsp_arg) \
  function bit nb_transport (REQ req_arg, output RSP rsp_arg); \
    ...
    end else nb_transport = imp.nb_transport(req_arg, rsp_arg); \
  endfunction
